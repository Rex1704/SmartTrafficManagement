...
def setTime():
    ...
    for i in range(noOfSignals):
        if i != currentGreen:
            vehiclesCount = len(vehicles[directionNumbers[i]][0]) + len(vehicles[directionNumbers[i]][1]) + \
                            len(vehicles[directionNumbers[i]][2]) - vehicles[directionNumbers[i]]['crossed']
    ...
    print("Updated nextGreen based on max vehicles:", directionNumbers[nextGreen])
...
def generateVehicles():
    ...
    while (True):
        ...
        if maxCounts[direction_number] > len(vehicles[directionNumbers[direction_number]][0]) + \
                len(vehicles[directionNumbers[direction_number]][1]) + len(vehicles[directionNumbers[direction_number]][2]):
